...
function newsFeed() {
    let newsFeedData = store.feeds; //getData(NEWS_URL);
    store.lastPage = newsFeedData.length / 10;
    const newsList = [];
    let template = `
    ...
    if(newsFeedData.length === 0) {
        newsFeedData = store.feeds = makeFeeds(getData(NEWS_URL));
    }
        ...
        <div class="p-6 ${newsFeedData[i].read ? 'bg-red-500' : 'bg-white'}  mt-6 rounded-lg shadow-md transition-colors duration-500 hover:bg-green-100">
            ...
            <div class="flex mt-3">
                <div class="grid grid-cols-3 text-sm text-gray-500">
                    ...
                    <div><i class="fas fa-clock mr-1"></i>${newsFeedData[i].time_ago}</div>
                </div>
            </div>
        </div>
    `);
    ...
    template = template.replace('{{__next_page__}}', store.lastPage > store.currentPage ? store.currentPage + 1 : store.lastPage);
...
function newsDetail() {
    ...
    let template = `
        <div class="bg-gray-600 min-h-screen pb-8">
            <div class="bg-white text-xl">
                <div class="mx-auto px-4">
                    <div class="flex jusify-between items-center py-6">
                        <div class="flex justify-start">
                            <h1 class="font-extrabold">Hacker News</h1>
                        </div>
                        <div class="items-center justify-end">
